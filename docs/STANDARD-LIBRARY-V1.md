...
| `std.tracing` | W3C Trace Context `traceparent`/`tracestate` admission policy (issue #193) | Partial: a pure, effect-free decision layer with no I/O and no `permit`/`uses`, admitting or refusing an inbound `traceparent` header before a caller creates, propagates, or exports a span. `traceparent_shape_admitted` requires the fixed 55-byte `version-trace_id-parent_id-trace_flags` shape, lowercase-hex-only fields, the three dash positions, `version` `ff` forbidden outright, and an all-zero trace-id or parent-id refused even though it is otherwise well-formed hex (W3C Trace Context §3.2.2-§3.2.2.4); `traceparent_sampled` reads the sampled bit from the trace-flags field's final hex digit. `tracestate_member_count_admitted` (32) and `tracestate_combined_length_admitted` (512) are the two aggregate `tracestate` budgets a caller can enforce without parsing a single list-member (§3.3-§3.3.1.5). `tracing_backend_audit::tracing_executes_on_all_three_backends` passes on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm. **This package does not generate a trace-id or parent-id, create/start/export a span, implement a sampler, or parse a single `tracestate` list-member's key/value**: those need a randomness capability and an exporter this package does not have, and nothing here is evidence that trace generation, propagation, sampling, or export exists |
