...
| Requirement | Status | Evidence owner | Complete when |
| --- | --- | --- | --- |
| Token-budgeted semantic context | Partial; bounded standalone and authenticated Project context use retained typed indexes without requiring full graph transfer. Compact CLI/service views replay losslessly; local measurements show byte savings but increased cl100k/o200k model-token counts, so token-efficient encoding remains open. | [Agent Context v2](AGENT-CONTEXT-V2.md), [compact projection measurements](COMPACT-SEMANTIC-PROJECTION-V1.md), [Economics](AGENT-ECONOMICS-V1.md), [Workspace Image](SEMANTIC-WORKSPACE-IMAGE-V1.md) | Exact model-token budgets, broader semantic edges, persistent indexing and representative measured savings are verified. |
