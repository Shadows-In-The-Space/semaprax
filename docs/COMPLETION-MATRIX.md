...
| Semantic Kernel v1 trust-reduction follow-on | Proposed programme opened; self-hosting rung 0 reached and evidenced, no higher rung reached | [Semantic Kernel v1](SEMANTIC-KERNEL-V1.md): a trusted-computing-base inventory, measured capacity ceilings, and a from-scratch Kernel-0 kernel language with a paper (non-mechanized) type-safety sketch. The compiler-HIR-to-Kernel-0 admission predicate (`reifies_into_kernel_zero`, `src/kernel_zero.rs`) is executable and tested (`kernel_zero_shaped_functions_reify` plus four negative cases) but inert -- calling it rejects nothing in any compiler diagnostic path. A from-scratch reference interpreter and differential test (`src/kernel_zero/{eval,differential}.rs`, `reference_interpreter_agrees_with_the_compiler_over_the_kernel_zero_corpus`) found zero disagreements against the compiler's own interpreter over a deterministic 74-program corpus (14 hand-written, 60 generated); this is evidence over a finite seeded corpus, not a proved theorem, and it exercises the interpreter backend only -- native and Wasm are unexercised. | Machine-check a proof (not only a paper sketch), wire the admission predicate into an actual diagnostic path or record its permanent inertness as a decision, extend differential faithfulness evidence to native and Wasm, and reach a self-hosting rung above 0. |
...
| Functions, closures, interfaces, implementations, generics | Partial; explicit forwarding, scoped argument inference v3, generic owned-record/result composition, compiler collections, private function values and scalar-snapshot/generic-loop closures are implemented. Graph, cleanup and ProgramRoot replay retain exact instance and mapping identities. A separately versioned bounded owning-capture closure profile (`own fn() -> R { body }`, exactly one lexical owned `Bytes` capture and one call) additionally executes end to end, but on the interpreter only: `hir::closure::owning_desugar` rewrites the verified construction-plus-its-call into a direct call before resolution (pinned by `owning_closure_desugars_to_a_direct_target_call` and `interpreter_executes_a_called_owning_closure_and_returns_the_target_result`), while `hir::resolve` and every other caller -- native C11 and Core Wasm codegen included -- are unchanged and still refuse with the stable `SPX-H006` diagnostic, pinned by `native_and_wasm_backends_refuse_the_owning_closure_with_the_stable_diagnostic` (`tests/language/function_values/owning_closures.rs`). | [Function Values v1](FUNCTION-VALUES-V1.md), [v2](FUNCTION-VALUES-V2.md), [Closures v1](CLOSURES-V1.md), [v2](CLOSURES-V2.md), [owning-capture closures v1](CLOSURES-OWNING-V1.md), [inference v3](GENERIC-ARGUMENT-INFERENCE-V3.md), [record composition](GENERIC-OWNED-RECORD-COMPOSITION-V2.md), [multiple owners](GENERIC-MULTI-OWNER-RECORDS-V1.md), [owned Result](GENERIC-OWNED-RESULT-V1.md), [forwarding](GENERIC-EXPLICIT-FORWARDING-V1.md), [compiler collections](GENERIC-COMPILER-COLLECTIONS-V1.md) | Complete constraints, broader inference and nested variant/Result composition, native/Wasm owning-capture execution (today refused by design, not merely unimplemented), general interfaces and implementations, public callable/generic ABI and generic package signatures. The admitted inference, closures and GEN-06 hosted evidence are already present. |
