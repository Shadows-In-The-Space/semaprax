...
| `std.metrics` | Counters, gauges, histograms, and effect-neutral instrumentation | Partial: scalar counter increment, bounded gauge clamping, histogram count and sum accumulation, and a fixed cardinality ceiling with a fail-closed admission sentinel. Its examples and conformance assert the exact results on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm; registered exporters, persistence, tracing, and general instrumentation remain Missing |
