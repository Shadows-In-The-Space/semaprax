...
65536 expression nodes and expression nesting depth 256. With the canonical
String lifecycle defined in [owned String variants v1](OWNED-STRING-VARIANTS-V1.md),
static String-drop work counts every `finalize_in_order` action across the
selected functions' CleanupPlan exits, capped at 262144. Repeated occurrences
on distinct exit paths each count. The standalone caller owns the `SPX-W111`
