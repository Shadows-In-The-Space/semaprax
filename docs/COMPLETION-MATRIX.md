...
| Reference application (`examples/task-service-project`) follow-on | Local evidence only; no automated regression gate | Composes the now-bundled `std.auth`+`std.jobs` dependencies into a full register/log-in/create-update/enqueue/complete/query/log-out scenario plus row-level-unauthorized and invalid-input rejection, in deterministic fixture mode; own source 12,003 bytes. `check`, `test`, and `run` were manually observed to pass (the directory's row in `examples/README.md`); no test under `tests/` or `src/**/tests.rs` executes those commands over this directory, so nothing regresses it in CI today. A four-package closure (adding `std.http`/`std.db` as real dependencies instead of local reimplementations) exceeds the Workspace Semantic Graph `builder_bytes` cap (`SPX-G171`, `MAX_BUILDER_BYTES` = 18,874,368) even under 12 KB of own source once ~10 functions are pulled across the four packages, and an authored `record` domain type is refused by `SPX-W121` (`Public Useful Data Export v1` forbids an authored aggregate alongside a web export) -- both are why the shipped app uses only two bundled packages and scalar facts. [Scaffold Service Template v1](PROJECT-SCAFFOLD-SERVICE-V1.md), [Authentication and Sessions v1](AUTHENTICATION-SESSIONS-V1.md), [Durable Jobs v1](DURABLE-JOBS-V1.md) | Add a `tests/` gate that runs `check`/`test`/`run` over this directory and fails the build on regression; `std.db`/`std.http` as real dependencies and an authored record domain type remain blocked by `SPX-G171`/`SPX-W121` until the capacity-ceilings row above is resolved. |
