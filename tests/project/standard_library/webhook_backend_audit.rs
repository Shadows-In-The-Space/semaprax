//! Issue #193: `std.webhook` is the inbound/outbound webhook decision layer,
//! added alongside `std.email` and following the shape
//! `log_redact_backend_audit.rs` established for its sibling policy
//! packages. Every package advertising a backend must actually exercise its
...
#[test]
fn webhook_executes_on_all_three_backends() {
    if cfg!(windows) {
        return;
    }
