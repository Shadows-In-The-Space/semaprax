...
pub fn build_web(program: &Program, output: &Path) -> Result<(), Diagnostic> {
    ...
    let uses_strings = functions_use_strings(
    ...
    let has_authored_aggregate = resolved.types.iter().any(|declaration| {
        matches!(
            &declaration.kind,
            ResolvedTypeDeclarationKind::Record { .. }
                | ResolvedTypeDeclarationKind::Class { .. }
    ...
    if uses_strings && !has_authored_aggregate {
        ...
        .with_help(
            "for standalone scalar String source, use --profile internal-strings-v1 with admitted scalar exports; aggregate String programs use the aggregate Web lowering",
        ));
    }
...
fn browser_runtime() -> &'static str {
    include_str!("wasm/browser_runtime.js")
}
