...
| `std.email` | Outbound-email address, header-injection, and envelope policy (issue #193) | Partial: a pure, effect-free decision layer with no I/O and no `permit`/`uses`, following `std.log.redact`'s shape. `value_is_header_safe` refuses CR, LF and NUL in any caller-controlled header value — the SMTP header-injection boundary — as a refusal rather than a sanitizer, so a caller can never ship a message whose bytes quietly differ from what it assembled; each of the three separator bytes is asserted individually, and a separator in the final position is asserted separately because a scan that stops one byte early passes every other case. `address_admitted` composes named predicates rather than one opaque judgement: exactly one `@` (counted, not split-on-first, so `a@b@c` is refused), header safety over the address itself, and the three RFC 5321 sizes (local part 1-64, domain 1-255, whole path 3-254) each asserted inclusively and at +1. `domain_shape_admitted` requires at least one dot and refuses a leading or trailing dot, so `user@localhost` is refused on purpose. `recipient_count_admitted` is one deployment-independent fan-out ceiling (64) and `envelope_is_complete` reports each missing part separately. Every function is scalar-in/scalar-out or `borrow Slice<u8>`-in/scalar-out, so neither a message nor a credential is copied into or out of the package. `email_backend_audit::email_executes_on_all_three_backends` passes on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm. **Sending email, MX resolution, SMTP, provider authentication and MIME rendering are Missing**: they need network and secret capability, and nothing here is evidence that outbound delivery exists |
| `std.webhook` | Inbound/outbound webhook replay-window, signature-envelope, and retry policy (issue #193) | Partial: a pure, effect-free decision layer with no I/O and no `permit`/`uses`. `timestamp_within_window` bounds a delivery in **both** directions against a 300-second window — a receiver that verifies a signature but ignores the timestamp accepts a valid signed request forever, and one that clamps only the past accepts a clock-skewed forgery indefinitely — with both edges and both +1 cases asserted. `signature_shape_admitted` admits exactly 64 lowercase hex digits, refusing a short, uppercase or non-hex header before it can reach any comparison routine; the hex alphabet's four boundary bytes and their four neighbours are asserted at the predicate itself. `payload_len_admitted` (65536) bounds an unauthenticated inbound body, `attempt_admitted` counts from 1 and refuses 0, and `backoff_seconds` doubles then saturates at 60 under a declared `ensures` range, defined only for admitted attempt numbers. `webhook_backend_audit::webhook_executes_on_all_three_backends` passes on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm. **Signature verification itself is deliberately absent, not merely unfinished**: a constant-time MAC comparison needs a primitive this language does not yet admit, and shipping a non-constant-time one would be worse than shipping none. Delivery, retry execution and queueing are likewise Missing |
| `std.tracing` | W3C Trace Context `traceparent`/`tracestate` admission, span-identity, and propagation policy (issue #193) | Partial: a pure, effect-free decision layer with no I/O and no `permit`/`uses`. Inbound: `traceparent_shape_admitted` requires the fixed 55-byte `version-trace_id-parent_id-trace_flags` shape, lowercase-hex-only fields, the three dash positions, `version` `ff` forbidden outright, and an all-zero trace-id or parent-id refused even though it is otherwise well-formed hex (§3.2.2-§3.2.2.4); `traceparent_sampled` reads the sampled bit from the trace-flags field's final hex digit. Outbound: `trace_context_fields_admitted` and `outbound_traceparent_fields_admitted` mirror the same shape and all-zero rules for a header a caller is assembling from separate fields rather than one already joined with dashes, with the all-zero trace-id/parent-id cases asserted on this outbound path too so admitting what inbound refuses cannot pass silently. `trace_context_fields_admitted_guarded` composes that structural check left-to-right with `std.log.redact.event_is_safe`, refusing each of the six caller-declared secret-bearing flags without accepting a malformed context merely because classification metadata is clean. It does not inspect bytes or `tracestate`, and the unguarded structural predicates remain available; an adapter must classify its payload and choose the guarded boundary. `child_context_admitted` decides whether a caller-supplied child span-id (never generated — this package has no randomness) yields a context admitted and consistent with its parent: same trace-id, a distinct parent-id, trace-flags carried unchanged. `trace_flags_sampled`/`sampling_decision_consistent` judge a standalone flags field's sampled bit and refuse only a sampled-to-unsampled downgrade, never inventing a sampling decision. `tracestate_key_shape_admitted`/`tracestate_value_shape_admitted` parse one already-isolated list-member's `simple-key` and value shape (§3.3.1.1-§3.3.1.2, `multi-tenant-key` out of scope), `tracestate_member_admitted` composes them over one `key=value` slice, and `tracestate_mutation_admitted` checks the front-of-list and no-duplicate rules pairwise against one sibling at a time (§3.3.1.3). `tracestate_member_count_admitted` (32) and `tracestate_combined_length_admitted` (512) remain the two aggregate `tracestate` budgets a caller can enforce without parsing. Baseline `tracing_backend_audit::tracing_executes_on_all_three_backends` covers the package on interpreter, native C11 `-O0`/`-O2`, and Core Wasm; the additive guarded predicate is separately exercised by the package conformance program and reference-application admission gate in this tranche. **This package does not generate a trace-id, parent-id, or span-id, create/start/export a span, implement a sampler, split a raw `tracestate` header into its list-members, or admit the `multi-tenant-key` form**: those need a randomness capability, an exporter, and a general delimiter scanner this package does not have, and nothing here is evidence that trace generation, propagation, sampling, or export exists |
| `std.metrics` | Counters, gauges, histograms, labels, and cardinality limits (issue #193) | Partial: scalar counter increment, bounded gauge clamping, histogram count and sum accumulation, and a label-safety and cardinality decision layer with no I/O and no `permit`/`uses`, in the same refuse-rather-than-sanitize shape as `std.email` and `std.http`. `label_name_is_valid` admits a closed allowlist — `[a-zA-Z_][a-zA-Z0-9_]*`, the Prometheus text-format label-name grammar — rather than a denylist, refusing a leading digit and any byte outside the set at length 1-64. `value_is_label_safe` refuses line feed, double quote and backslash (the exact bytes the Prometheus text format's own value escape covers) plus CR and NUL (the same defensive pair `std.http` and `std.email` refuse alongside their own format's separator), each asserted individually and in the final byte position as well as the middle; `label_value_len_admitted` bounds a value to 128 bytes, empty admitted. `label_count_admitted` is one deployment-independent per-series label-count ceiling (32), tested at zero, the bound and one past it. `label_admitted` composes the name and value judgements, and `try_admit_labeled_series` is the cardinality admission that actually sees the label — a bad label is refused with the fixed `-1` sentinel even when the existing count is far under `cardinality_limit()`, replacing the prior `try_admit_series`-only admission that counted abstract series without inspecting a single label byte. `byte_is_label_value_unsafe` is written as a `match` rather than a chained `||`, avoiding the `SPX-H006` cleanup-replay budget `std.http.byte_is_tchar` hit at a comparable size. Its examples and conformance assert the exact results on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm; registered exporters, persistence, tracing, and general instrumentation remain Missing |
...
Every direct `standard_library` module that declares tests now has a
`std-library-depth` selector in `.github/workflows/ci.yml`; the execution-matrix
selector guard refuses an unselected owning module. The workflow selection is
local configuration until a hosted CI run reports its exact-head verdict. Hosted
release and branch-governance evidence remain separate from this package gate.
