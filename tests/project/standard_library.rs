...
"#,
            ...
            for (role, module_bytes) in [
                ...
                let arena = role == "tests" && (cursor_case || matches!(package.module.as_str(), "std.data.json.dec" | "std.io" | "std.io.lines" | "std.path.value" | "std.path.normalize" | "std.log.redact" | "std.email" | "std.webhook" | "std.tracing") || (package.module == "std.format" && formatting::uses_byte_arena(&manifest)) || (package.module == "std.log" && logging::uses_byte_writes(&manifest)));
                if role == "tests" {
