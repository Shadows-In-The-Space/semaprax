...
const LABELS: [&str; 14] = [
    "FrameValidated", "LeafAllocationStarted", "LeafAllocationCommitted",
...
fn closed_keys(value: &Value, keys: &[&str]) {
    let actual: BTreeSet<_> = value.as_object().expect("expected object").keys()
        .map(String::as_str).collect();
    assert_eq!(actual, keys.iter().copied().collect::<BTreeSet<_>>(), "closed manifest keys");
}
...
fn injection(value: &Value) -> Option<usize> {
    if value.is_null() {
        ...
        Some(LABELS.iter().position(|label| Some(*label) == value.as_str())
...
fn digest(direction: &str, bytes: &[u8]) -> String {
    ...
    hash.update(bytes);
    format!("sha256:{:x}", semaprax::digest_hex::LowerHex(hash.finalize()))
}
...
fn parse(text: &str) -> Vec<FixtureCase> {
    ...
    closed_keys(&value, &["schema", "profile", "carrier_format",
        "normalized_result_format", "applicable_engines", "cases"]);
    assert_eq!(value["schema"], SCHEMA);
    assert_eq!(value["profile"], "flat-owned-bytes-reference-fixture.v1");
    assert_eq!(value["carrier_format"], "u64le-leaf-count-and-u64le-length-framed-bytes");
    assert_eq!(value["normalized_result_format"], "u64le-length-framed-bytes-without-leaf-count");
    assert_eq!(value["applicable_engines"], serde_json::json!([
        "interpreter", "core-wasm-model", "native-c11-O0", "native-c11-O2"]));
    let cases = value["cases"].as_array().expect("expected cases");
    assert!(!cases.is_empty() && cases.len() <= 64, "case count bound");
    let mut ids = BTreeSet::new();
    cases.iter().map(|case| {
        closed_keys(case, &["case_id", "description", "input_leaves",
            "failure_injection_id", "compound_cleanup_injection", "expected_accepted",
            "expected_primary_status", "expected_endpoint_invoked", "input_carrier_digest",
            "expected_result_carrier_digest", "expected_final_live_allocations",
            "expected_final_live_handles", "expected_final_live_bytes"]);
        let id = case["case_id"].as_str().expect("expected case ID");
        assert!(!id.is_empty() && id.len() <= 100 && id.bytes().all(|c|
            c.is_ascii_alphanumeric() || c == b'_'), "invalid case ID");
        assert!(ids.insert(id), "duplicate case ID");
        assert!(case["description"].as_str().is_some_and(|s| !s.is_empty() && s.len() <= 512));
        ...
        for recipe in recipes {
            ...
            total = total.checked_add(leaf.len().checked_mul(count).unwrap()).unwrap();
        ...
        assert_eq!(case["input_carrier_digest"], digest("input", &input), "input digest");
        let accepted = case["expected_accepted"].as_bool().expect("expected boolean");
        ...
        assert_eq!(case["expected_result_carrier_digest"], expected_result, "result digest");
        let injected = injection(&case["failure_injection_id"]);
        let cleanup = injection(&case["compound_cleanup_injection"]);
        assert!(cleanup.is_none_or(|label| label >= 12), "not a cleanup event");
        assert!(cleanup.is_none() || injected.is_some(), "cleanup needs primary injection");
        let within_bounds = leaves.len() <= 256 && leaves.iter().all(|leaf| leaf.len() <= 65536);
        let invoked = within_bounds && injected.is_none_or(|label| label >= 7);
        assert_eq!(case["expected_endpoint_invoked"], invoked, "endpoint expectation");
        for field in ["expected_final_live_allocations", "expected_final_live_handles",
            "expected_final_live_bytes"] {
            assert_eq!(case[field].as_u64(), Some(0), "nonzero terminal resources");
        }
        FixtureCase { case_id: id.to_owned(), input_leaves: leaves, injection: injected,
            cleanup_injection: cleanup, accepted, status }
    }).collect()
}
...
fn settlement_manifest_rejects_tampering_and_noncanonical_bytes() {
    ...
    for field in ["case_id", "input_carrier_digest", "expected_result_carrier_digest",
        "failure_injection_id", "expected_primary_status", "expected_endpoint_invoked"] {
        let mut changed = original.clone();
        changed["cases"][0][field] = Value::String("invalid value".to_owned());
        let text = format!("{}\n", serde_json::to_string(&changed).unwrap());
        assert!(std::panic::catch_unwind(|| parse(&text)).is_err(), "accepted mutation: {field}");
    }
    for text in [MANIFEST.trim_end().to_owned(), format!(" {MANIFEST}"),
