...
| ABI-09: public generic programme | Existing public profiles remain separately closed; internal generic implementation does not change their signatures. PG-1 through PG-8 (grammar, template/argument identities, compatibility, candidate delta, generated consumers, hostile replay, and cross-engine settlement) are hosted green together for one exact implementation commit (`7def8fb1…`, run [35433295593](https://github.com/wavect/semaprax/actions/runs/35433295593)), and PG-9 was decided 2026-09-19: `unsupported`, `unpublished` (see [Public Generic Ownership milestone](PUBLIC-GENERIC-OWNERSHIP-MILESTONE-V1.md)). The internal [`public-generic-wasm-provider.v1`](PUBLIC-GENERIC-WASM-PROVIDER-TARGET-V1.md) profile now retains one compiler-derived endpoint and emits its deterministic zero-import full-lifecycle Core Wasm provider; legacy publication routes remain closed. | Replace the generated TypeScript package's legacy flat carrier codec with canonical descriptor-bound carrier frames, then execute that caller and the remaining physical adapters against checked endpoint semantics (#162/#229). Separately run issue #164's exact-head freeze protocol; the cited hosted run predates current head. None of these steps changes PG-9 without a new explicit support decision. |
