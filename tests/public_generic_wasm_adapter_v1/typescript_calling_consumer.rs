//! Generated TypeScript package tests for the public-generic Wasm caller.
//! The production runtime now requires the compiler-owned full provider ABI;
//! the historical hand-assembled endpoint-only module remains only to prove
//! that legacy host-owned bookkeeping artifacts are rejected. Successful
//! compiler-provider execution is owned by `compiler_provider_artifact`.
//!
...
/// standalone external package with the repository-pinned `tsc`, then prove
/// the retired host-owned reference module is rejected by the new exact
/// module-owned ABI before instantiation. The compiler-owned provider's real
/// success lifecycle is covered by `compiler_provider_artifact`.
#[test]
fn generated_typescript_consumer_rejects_the_retired_host_owned_reference_module() {
    if !node_available() {
        eprintln!("skipping: node is not available on PATH");
        return;
    }
    ...
    // Sanity-check the compiler-owned provider artifact with Node's own
    // validator before trusting any generated-consumer result that follows.
    ...
    assert!(
        ...
        "the retired reference module failed basic Wasm validation: stdout={} stderr={}",
    ...
    fs::write(
        package_root.join("test/legacy-rejection.mjs"),
...
"#,
    ...
    let rejection = run(
        &mut node_from(
            &package_root,
            "test/legacy-rejection.mjs",
            Some("../reference.wasm"),
        ),
        "node test/legacy-rejection.mjs",
    );
    assert!(
        rejection.status.success(),
        "the generated client did not reject the retired host-owned module: stdout={} stderr={}",
