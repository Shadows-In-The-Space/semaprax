...
const SERVICE_README: &str = "# {{name}}\n\nA small multi-user task-tracking service created by SEMAPRAX, composed from seven bundled standard-library decision layers: register, log in, validate a request and migration, create a domain record, enqueue and complete a background job, admit a bounded metric and exporter batch, query its status, log out, and reject an unauthorized or invalid request. Every step is deterministic fixture mode -- no socket, no file, and no real clock are touched. `std.db`, `std.http`, `std.metrics`, `std.export.policy`, and `std.tracing` contribute pure admission or shape checks only; they do not open storage, transport, or telemetry.\n\n```sh\nsemaprax check .\nsemaprax test .\nsemaprax run .\n```\n\nRead `AGENTS.md` before editing the source, whether you are a person or a\ncoding agent: it lists the commands, the rules that differ from other\nlanguages, and this template's bundled dependencies.\n";
const SERVICE_DEPENDENCY_GUIDE: &str = "\n## This template's dependencies\n\n`{{module}}.core` imports `std.auth` (session lifecycle, password-hash\npolicy bounds), `std.db` (identifier, migration, and transaction decisions),\n`std.http` (request-line admission), `std.jobs` (claim/lease/retry/idempotency\nstate machines), `std.metrics` (guarded metric-series admission),\n`std.export.policy` (bounded batch and sink admission), and `std.tracing`\n(trace-context shape plus caller-classified secret safety) by stable `@id`,\ndeclared in `semaprax.toml`'s `[dependencies]` table. All seven are pure\ndecision layers: they neither hash a password, open a socket or database, nor\nemit a log or span. A real deployment performs classification, hashing,\nsigning, storage, transport, and any telemetry emission outside this decision.\n";
