...
/// A multi-user service composed from three bundled standard-library decision
/// layers (`std.auth`, `std.jobs`, `std.tracing`); see
...
const SERVICE_README: &str = "# {{name}}\n\nA small multi-user task-tracking service created by SEMAPRAX, composed from three bundled standard-library decision layers: register, log in, create a domain record, enqueue and complete a background job, query its status, log out, and reject an unauthorized or invalid request. Every step is deterministic fixture mode -- no socket, no file, and no real clock are touched. `std.tracing` contributes only pure trace-context and secret-safety validation; it does not emit telemetry.\n\n```sh\nsemaprax check .\nsemaprax test .\nsemaprax run .\n```\n\nRead `AGENTS.md` before editing the source, whether you are a person or a\ncoding agent: it lists the commands, the rules that differ from other\nlanguages, and this template's bundled dependencies.\n";
const SERVICE_DEPENDENCY_GUIDE: &str = "\n## This template's dependencies\n\n`{{module}}.core` imports `std.auth` (session lifecycle, password-hash\npolicy bounds), `std.jobs` (claim/lease/retry/idempotency state machines),\nand `std.tracing` (trace-context shape plus caller-classified secret safety)\nby stable `@id`, declared in `semaprax.toml`'s `[dependencies]` table. All\nthree are pure decision layers: they neither hash a password, open a socket,\nor emit a log or span. A real deployment performs classification, hashing,\nsigning, storage, transport, and any telemetry emission outside this decision.\n";
