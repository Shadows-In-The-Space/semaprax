//! Checkpoint-store adapter for the generic durable policy journal v2.
//!
...
use crate::live_invocation::policy_journal::{
    PolicyAttemptOutcome, PolicyJournal, PolicyJournalError,
};

pub(crate) struct PolicyCheckpointJournal<'a> {
    store: &'a mut dyn CheckpointStore,
    journal: PolicyJournal,
    generation: u64,
}

impl<'a> PolicyCheckpointJournal<'a> {
    pub(crate) fn new(store: &'a mut dyn CheckpointStore, journal: PolicyJournal) -> Self {
        Self {
            store,
            journal,
            generation: 0,
    ...
    pub(crate) fn journal(&self) -> &PolicyJournal {
        &self.journal
    }
    ...
    fn persist(&mut self) -> Result<(), String> {
        ...
        self.store
            .commit(next, &self.journal.render())
            .map_err(store_error)?;
...
impl RetryAttemptJournal for PolicyCheckpointJournal<'_> {
