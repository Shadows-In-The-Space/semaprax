...
interpreter on the bound deterministic stages of one fixture
(`every_stage_executor_agrees_on_every_deterministic_stage_of_one_source_program`,
`src/agent_lifecycle/tests.rs`) -- local, `clang`/`node`-gated evidence for
stage bodies only. Budgets, cancellation, effect dispatch and evidence
settlement still run on the interpreter alone (#142/#143/#182).
...
| Reference application (`examples/task-service-project`) follow-on | Local evidence; automated regression gate now exists | Composes the bundled `std.auth`+`std.jobs`+`std.tracing` dependencies into a full register/log-in/create-update/enqueue/complete/query/log-out scenario plus row-level-unauthorized, invalid-input, trace-context, and secret-classification decisions, in deterministic fixture mode; own source 15,277 bytes. `check`, `test`, and `run` now run under `tests/useful_data/task_service_project.rs::check_test_and_run_pass_on_the_interpreter` (asserted via `ProjectSnapshot::check`/`execute_entry`/`execute_test`, plus that the workspace carries the bundled `std.auth`/`std.jobs`/`std.tracing` source rather than hand-vendored copies) and `the_declared_web_exports_are_reachable_public_functions` asserts the `[exports].web` roots resolve in the retained revision's semantic graph, the same shape `agent_response_project.rs` asserts for its sibling. A five-package dependency set (adding `std.http`/`std.db` as real dependencies instead of local reimplementations) still must remain within the Workspace Semantic Graph `builder_bytes` cap (`SPX-G171`, `MAX_BUILDER_BYTES` = 18,874,368); the earlier unpruned four-domain-package experiment exceeded that cap once ~10 functions were pulled, and an authored `record` domain type is refused by `SPX-W121` (`Public Useful Data Export v1` forbids an authored aggregate alongside a web export) -- both are why the shipped app keeps `std.db`/`std.http` local and uses scalar facts. The native/Wasm execution gap is bounded precisely: `entry_and_conformance_return_zero_on_interpreter_native_and_wasm` runs entry and conformance closures on the interpreter and native C11 at `-O0`/`-O2`, and runs the conformance closure on Core Wasm under Node; it does not execute the entry closure on Wasm. `stable_id_rename_inspect_and_context_succeed_preview_pins_the_comment_precondition` walks issue #194 step 7's inspect/context/impact/preview chain against this project. Universal Semantic Transaction v1's `rename_display_name` used to require the complete workspace -- this project's own three modules and every bundled dependency source actually reached -- to be comment-free, which no consumer of a commented bundled package could ever satisfy by editing its own source. Issue #274 fixed that at the cause: `ProjectCandidate::apply`'s `materialize` now preserves an untouched program's exact base bytes instead of re-deriving every source through the comment-dropping canonical formatter, and `src/project/semantic_transaction/canonical_sources.rs` requires comment-free canonical source only of the sources a transaction actually rewrites (checked directly: the same preview succeeds against the same commented `std.auth` + `std.jobs` + `std.tracing` closure once this project's own modules are copied out and stripped of comments). `preview` still fails closed with `SPX-G525` on the checked-in project, now for a caller-actionable reason: `src/core.spx` owns the renamed declaration and is therefore the source the rename rewrites, and it carries 31 comment lines of its own. [Scaffold Service Template v1](PROJECT-SCAFFOLD-SERVICE-V1.md), [Authentication and Sessions v1](AUTHENTICATION-SESSIONS-V1.md), [Durable Jobs v1](DURABLE-JOBS-V1.md) | `std.db`/`std.http` as real dependencies and an authored record domain type remain blocked by `SPX-G171`/`SPX-W121` until the capacity-ceilings row above is resolved. The rename/apply/retest half of step 7 remains undemonstrated on this project because its own `src/core.spx` is deliberately comment-documented; the bundled-dependency half of that blocker is closed (issue #274). A deterministic, offline OCI Image Layout emitter now exists (`semaprax build --target oci`, `crates/semaprax-oci-package`, [OCI Deployable Artifact v1](OCI-DEPLOYABLE-ARTIFACT-V1.md)), but only for the Project v1 scalar profile; checked in this session, this application's manifest does **not** qualify -- it declares `schema = "semaprax.manifest.v1"` (the extensible Useful Data Export v1 table-format schema its `[dependencies]`/`[exports]` sections require) under the `useful-data.v1` profile, not the frozen `semaprax.project.v1` schema's `ScalarV1` profile the oci target is wired to, so `semaprax build --target oci examples/task-service-project` refuses closed with `SPX-J142`; the two schemas are mutually exclusive (the flat `semaprax.project.v1` layout has no `[dependencies]` table), not a gap in this project's authoring. `std.tracing` is now a checked-in pure trace-context and caller-classified secret-redaction policy dependency. `std_tracing_and_its_redaction_dependency_fit_the_real_reference_application` confirms the selected `std.tracing`/`std.encoding`/`std.log.redact` closure, and the application’s valid/malformed/secret conformance cases execute through the interpreter, native C11, and Core Wasm test gate, while entry execution remains interpreter/native-only. This does **not** add `std.log`, log or span emission/export, an observability profile, or any other host capability; `std.log` remains deliberately absent.  |
| Semantic Kernel v1 trust-reduction follow-on | Proposed programme opened; self-hosting rung 0 reached and evidenced, no higher rung reached | [Semantic Kernel v1](SEMANTIC-KERNEL-V1.md): a trusted-computing-base inventory, measured capacity ceilings, and a from-scratch Kernel-0 kernel language, originally with only a paper (non-mechanized) type-safety sketch. [Kernel-0 proof mechanization](KERNEL-PROOF-MECHANIZATION-V1.md) mechanized part of that sketch in Lean 4 (`proofs/kernel0-lean/Kernel0.lean`): a fully proved Progress trichotomy for the scalar-and-`if` fragment and a fully proved Preservation for the *entire* language including `Let` and non-recursive `Call`, zero `sorry`/`admit`/custom axioms, `#print axioms` reporting only `propext`/`Quot.sound`. Issue #188 gave that proof an executable gate (`scripts/kernel0-lean-gate.py`, wired into `scripts/quality.sh full` and a `kernel0-lean-proof-gate` CI job): headline-theorem presence and byte-exact pinned signatures plus a comment-aware `sorry`/`admit`/`axiom` token scan run unconditionally with no Lean toolchain needed, and a cold `lake build` plus `#print axioms` axiom-set audit run wherever `lake` is on PATH. The `kernel0-lean-proof-gate` CI job now provisions the pinned toolchain itself -- a sha256-pinned `elan` archive plus the exact version named by `proofs/kernel0-lean/lean-toolchain` -- and runs both Lean gates with `--require-kernel`, so an absent or mismatched kernel fails the job instead of taking its skip path; the job is in `release-gate`'s blocker set. The earlier exclusion rested on reading AGENTS.md's no-build-time-network invariant as barring toolchain provisioning in a setup step, which the same workflow already performs for Rust, Node and TypeScript; what the invariant bars is a *build* reaching the network, and `lake build` here resolves no dependency (`lake-manifest.json` declares `"packages": []`). Every kernel result quoted in this matrix was nonetheless produced on a developer host: the gate is wired, and a hosted verdict becomes quotable per commit once one exists. This mechanization does not cover termination (Kernel-0's call-graph acyclicity) and has no connection to the compiler's real HIR -- see that document's "What this does not tell you". The compiler-HIR-to-Kernel-0 admission predicate (`reifies_into_kernel_zero`, `src/kernel_zero.rs`) is executable and tested (`kernel_zero_shaped_functions_reify` plus four negative cases) but inert -- calling it rejects nothing in any compiler diagnostic path. **Decided, this session, as permanently inert by design rather than left as open wiring work:** [Semantic Kernel v1](SEMANTIC-KERNEL-V1.md)'s "Reification: HIR to Kernel-0, and its unproved edge" records the reasoning -- a mandatory refusal wired from a proof-fragment-membership predicate would silently narrow the admitted language (the mistake issue #208's session made and had rejected), and the non-mandatory alternative (a queryable `AgentContextFilter` facet or CLI verb) would be a new versioned, deterministic wire-format/CLI commitment for a boolean already derivable from facts the compiler exposes today (existing `contracts`/`ownership`/`effects`/`types` context facets). A from-scratch reference interpreter and differential test (`src/kernel_zero/{eval,differential}.rs`, `reference_interpreter_agrees_with_the_compiler_over_the_kernel_zero_corpus`) found zero disagreements against the compiler's own interpreter over a deterministic 74-program corpus (14 hand-written, 60 generated). **Extended, this session, to native C11 and Core Wasm** (`src/kernel_zero/differential/cross_backend.rs`, `native_c11_and_core_wasm_agree_with_the_kernel_zero_reference_interpreter_over_the_corpus`): the identical corpus and seed, run through `codegen::emit_hir_c` compiled with the local `clang` at `-O0`/`-O2` and through `wasm::build_web_with_scalar_exports`'s Public Scalar Export Profile v1 under Node, each compared against the from-scratch reference interpreter -- 591 comparisons (197 samples x native `-O0` x native `-O2` x Core Wasm), zero disagreements. This remains evidence over one finite seeded corpus, not a proved theorem, for both the interpreter and the native/Wasm extension; it says nothing about any program outside the corpus. **Issue #188's seeded-defect audit** then confirmed the Lean proof itself, not only its admission-predicate and token-scan layers, rejects real unsoundness: a decoupled-`if`-branch-type typing rule and an off-by-one `Call`-beta substitution base each broke `lake build` with a genuine `Type mismatch` inside `Kernel0.preservation`'s exact affected case (not merely a `sorryAx`/token-scan catch), both reverted after confirming and reproduced locally (`lake` is on this host's `elan` toolchain once `~/.elan/bin` is on `PATH`, contrary to a bare `which lake`). The same audit also found and fixed a real, previously unnoticed fidelity gap in the mechanization itself, independent of any seeding: `evalArith`'s `mod` case had no `i64::MIN`/`-1` exclusion (unlike `div`, whose quotient overflow `inRange` already rejects), so the proof's modeled `mod` silently disagreed with the real system's `RemainderOverflow` fault at that one boundary (`src/kernel_zero/eval.rs`'s `checked_rem`, already exercised by a passing differential-corpus case); fixed with an explicit exclusion mirroring `div`'s, requiring zero proof changes since Progress/Preservation are generic over `evalArith`'s `Option` shape, not its formula. | Quote a hosted verdict from the now-wired `kernel0-lean-proof-gate` blocker once one exists; extend the mechanized proof to termination and to a verified HIR translation; grow the differential corpus (larger or adversarially-chosen, beyond one seeded generator's 60 programs) across all three backends; reach a self-hosting rung above 0. |
