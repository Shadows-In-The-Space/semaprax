...
| Reference application (`examples/task-service-project`) follow-on | Local evidence; automated regression gate now exists | Composes the bundled `std.auth`+`std.jobs`+`std.tracing` dependencies into a full register/log-in/create-update/enqueue/complete/query/log-out scenario plus row-level-unauthorized, invalid-input, trace-context, and secret-classification decisions, in deterministic fixture mode; own source 15,277 bytes. `check`, `test`, and `run` now run under `tests/useful_data/task_service_project.rs::check_test_and_run_pass_on_the_interpreter`, while the declared web exports are asserted reachable in the retained semantic graph. The graph budget is now 67,108,864 bytes, removing the former `SPX-G171` capacity blocker for measured bounded closures; `SPX-W121` still forbids an authored aggregate alongside a Public Useful Data web export. Native C11 `-O0`/`-O2` and Core Wasm execute the conformance closure; the entry closure remains interpreter/native-only. Issue #274 preserves untouched commented dependency bytes during semantic transactions, while this project's own commented `src/core.spx` still causes its rename preview to fail closed with `SPX-G525`. The deterministic offline OCI target remains limited to the mutually exclusive Project v1 scalar schema, so this Useful Data v1 manifest is refused with `SPX-J142`. `std.tracing` remains a pure checked-in policy dependency; no log/span emission or host capability is claimed. [Scaffold Service Template v1](PROJECT-SCAFFOLD-SERVICE-V1.md), [Authentication and Sessions v1](AUTHENTICATION-SESSIONS-V1.md), [Durable Jobs v1](DURABLE-JOBS-V1.md) | Adding `std.db`/`std.http` still requires a deliberate application integration and `SPX-W121` still blocks an authored record export; graph capacity alone is no longer the blocker. The rename/apply/retest and OCI profile gaps remain as described. |
